...
export default function LawyerHiringHistoryPage() {
  ...
  const handleAction = async (requestId, decision) => {
    // decision will be 'active' (for accept) or 'rejected'
  ...
  return (
    <div className="space-y-6 bg-white text-black dark:bg-black dark:text-white">
      <div>
        <h1 className="text-xl font-bold text-white">Client Retainer Inquiries</h1>
        <p className="text-xs text-zinc-500 mt-0.5">Manage and review your inbound professional hiring proposals.</p>
      </div>

      <div className="dark:bg-[#0d0d0d] border border-neutral-800 rounded-2xl overflow-hidden shadow-xl">
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-left text-sm">
            <thead>
              <tr className="border-b bg-white dark:border-neutral-900 bg-neutral-950 text-zinc-400 font-medium text-xs tracking-wider uppercase">
            ...
            <tbody className="divide-y divide-neutral-900/60 text-zinc-300">
              {isDataLoading ? (
                ...
                requests.map((request) => (
                  <tr key={request._id} className="hover:bg-neutral-900/20 transition-colors">
                    <td className="p-4 font-bold  text-black dark:text-white">
                      {/* 🛠️ Fallback chain checking typical Mongoose populates or direct fields */}
                    ...
                    <td className="p-4">
                      <span className={`text-[10px] txt-black font-bold uppercase tracking-wider px-2 py-0.5 border rounded-md ${
                        request.status === 'active' || request.status === 'accepted' ? 'bg-emerald-950/40 text-black dark:text-emerald-400 border-emerald-900/50' :
                        request.status === 'rejected' ? 'bg-red-950/40 text-red-400 border-red-900/50' :
                      ...
                      {(!request.status || request.status === 'pending') ? (
                        <div className="flex items-center justify-end gap-2">
                          <button
                            ...
                            className="h-8 px-3 rounded-lg border border-neutral-800 text-zinc-400 hover:text-emerald-400 hover:bg-neutral-900 text-xs font-bold transition-all inline-flex items-center gap-1"
                          >
                        ...
                        <span className="text-xs text-zinc-600 italic px-3">Resolved</span>
                      )}
                    </td>
                  </tr>
                ))
