...
export default function LawyerTransactionHistoryPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_URL}/transactions/lawyer/${lawyer.id}`)
      .then((res) => res.json())
      .then((data) => {
        if (!isMounted) return;
  ...
  return (
    <div className="space-y-6 p-6">
      <div>
        <h1 className="text-xl font-bold text-black dark:text-white">Earnings & Licensing Statements</h1>
              ...
              <tbody className="divide-y divide-neutral-200 dark:divide-neutral-900 text-zinc-700 dark:text-zinc-300">
                {transactions.map((tx) => (
                  <tr key={tx._id} className="hover:bg-neutral-50 dark:hover:bg-neutral-900/20 transition-colors">
                    <td className="p-4 font-mono text-xs text-zinc-400">{tx._id}</td>
                    <td className="p-4 text-xs font-bold">
                      <span className={tx.priceType === 'lawyer_license' ? 'text-purple-400' : 'text-emerald-400'}>
                        {tx.priceType === 'lawyer_license' ? 'Registry Placement Fee' : 'Inbound Retainer Fee'}
                      </span>
                    </td>
                    <td className="p-4 text-xs font-mono">{new Date(tx.createdAt).toLocaleDateString(undefined, { dateStyle: 'medium' })}</td>
                    <td className="p-4">
